...
//
// ⚠️ 네이티브에서만 부른다. app.config.ts 의 web.output="static" 때문에 expo start 는
//    누가 http://localhost:8081 을 열면 이 파일을 **Node 에서** 한 번 실행하는데,
//    그때는 각 SDK 의 .web 구현이 잡힌다. initializeKakaoSDK 의 웹 구현은 두 번째
//    인자로 web.javascriptKey·restApiKey 를 요구하는 async 함수라, 여기서처럼 앱키만
//    넘기면 거부된다. module scope 라 .catch() 를 걸 곳이 없어 unhandled rejection 이
//    되고, CLI 가 --unhandled-rejections=throw 로 돌아 dev 서버째 죽는다.
//    (GoogleSignin.configure 도 웹에서는 not-implemented 경고만 찍는 빈 껍데기다.)
if (Platform.OS !== "web") {
