 ...
 *
 * 위치는 GPS 보정(열차 지연 반영)에만 쓴다. 거절해도 풍경 알림은 예정 시각대로
 * 온다 — 그래서 문구도 "정확히 맞추려면" 이지 "받으려면" 이 아니다.
 *
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      ask({
        ...
        message:
          "열차가 늦어져도 창밖 풍경 알림 시각을 정확히 맞추려면 위치 권한이 필요해요. 탑승 중에만 사용해요.",
        confirmLabel: "권한 허용",
        cancelLabel: "나중에",
        onConfirm: async () => {
          ...
          notify({
            title: "권한이 꺼져 있어요",
            message:
              "설정 > 앱 > 권한 > 위치에서 허용하면 열차 지연을 반영해 풍경 알림 시각을 맞춰드려요.",
          });
        },
      });
    })();
